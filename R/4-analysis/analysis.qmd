...
# read the df
openalex_pubmed_papers <- fread(here("data/3-merged/openalex_pubmed_papers_merged_2025-04-29.csv"))
nrow(openalex_pubmed_papers)#105605

papers_2024 <- openalex_pubmed_papers %>%
  filter(publication_year == "2024")
nrow(papers_2024)#18437
```
...
ggplot(plot_data, aes(x = Source, y = Count, fill = Source)) +
  geom_bar(stat = "identity", width = 0.6) +
  geom_text(aes(label = scales::comma(Count)), 
            vjust = -0.5, size = 4) +
  scale_fill_viridis_d(option = "D") +
  scale_y_continuous(labels = scales::comma) +  # Format y-axis labels as plain numbers
  labs(
...
openalex_pubmed_affil <- fread("data/3-merged/openalex_pubmed_affil.csv")

# convert iso two letter country code into country name
openalex_pubmed_affil <- openalex_pubmed_affil %>%
  mutate(
    COUNTRY = ifelse(country_code == "XK", "Kosovo", COUNTRY),  # Handle XK first
    COUNTRY = ifelse(is.na(COUNTRY) & !is.na(country_code), 
                     suppressWarnings(countrycode(country_code, "iso2c", "country.name")),  # Suppress XK warning
                     COUNTRY)
  )

openalex_pubmed_affil <- openalex_pubmed_affil %>%
  mutate(
    institution_display_name = case_when(
      institution_display_name == "Ludwig Maximilians University (LMU)" ~ "Ludwig-Maximilians-Universität München",
      institution_display_name == "Technical University of Munich (TUM)" ~ "Technical University of Munich",
      institution_display_name == "Free University of Berlin (FU Berlin)" ~ "Freie Universität Berlin",
      institution_display_name == "Charité University Medicine Berlin" ~ "Charité - Universitätsmedizin Berlin",
      institution_display_name == "Humboldt University of Berlin (HU Berlin)" ~ "Humboldt-Universität zu Berlin",
      ...
      institution_display_name == "University of Tübingen (Uni Tübingen)" ~ "University of Tübingen",
      TRUE ~ institution_display_name  # Keep original if no match
    )
  )
...
na_country <- openalex_pubmed_affil %>% filter(is.na(COUNTRY)) %>% nrow()
...
1. Synonym Table Matching: We first match the affiliations with our synonym table, 88% of the affiliations received a match at this step.
2. ROR API Matching: Unmatched affiliations strings were passed through the ROR API, a cumulative of 94% of the affiliations were matched until this step.
3. ChatGPT Matching: The remaining unmatched affiliation strings were processed using ChatGPT to extract details such as institution, country and city. 
4. Some of the extracted institution names were matched with ROR institution names for standardization.

* Full confidence match: chosen=True & score=1 ~44 %(9590/21982)
* High confidence match: chosen=False & score=1 OR chosen=False & score=(0.9:0.99) ~56% - obvious mistakes happen when score=0.91
...
top_country <- openalex_pubmed_affil %>%
  ...
  distinct(primary_id, COUNTRY, .keep_all = TRUE) %>%
...
country_year_counts <- country_year_data %>%
  distinct(primary_id, COUNTRY, publication_year) %>%
  count(COUNTRY, publication_year)
...
all_papers_count <- openalex_pubmed_papers %>%
  mutate(
    num_citations = coalesce(num_citations_cf, num_citations_oa, 0),
  ) %>%
...
check_papers_author <- openalex_pubmed_affil %>%
  mutate(
    display_name_filled = case_when(
      !is.na(au_display_name) ~ au_display_name,
  ...
  group_by(primary_id) %>%
  summarise(
    author_list = paste(unique(display_name_filled[!is.na(display_name_filled)]), collapse = "; "),
    .groups = "drop"
  )

# Step 3: Identify papers with at least one German affiliation
german_primary_ids <- openalex_pubmed_affil %>%
  filter(COUNTRY == "Germany" & !is.na(primary_id)) %>%
  distinct(primary_id) %>%
  pull(primary_id)
...
check_papers <- openalex_pubmed_affil %>%
  group_by(primary_id) %>%
  summarise(
    all_countries = paste(unique(COUNTRY[!is.na(COUNTRY)]), collapse = "; "),  # Concatenate unique countries
    .groups = "drop"
  )
...
journals <- c("The Lancet Global Health", "Bmj Open", "Bmj Global Health")
...
top_institutions_global <- openalex_pubmed_affil %>%
  filter(!is.na(institution_display_name)) %>%
  distinct(primary_id, institution_display_name, COUNTRY, .keep_all = TRUE) %>%
  count(institution_display_name, COUNTRY, sort = TRUE)
...
In case you wonder why you haven't heard much about London School of Hygiene & Tropical Medicine and wonder what papers they produce, here're 50 papers produced by them, chosen randomly. 
...
institution_collab <- openalex_pubmed_affil %>%
  ...
  filter(!is.na(institution_display_name), COUNTRY != "Germany")  # exclude German institutions

# Count how many German papers each institution appears in
top_collab_institutions <- institution_collab %>%
  group_by(institution_display_name, COUNTRY) %>%
  summarise(paper_count = n_distinct(primary_id), .groups = "drop") %>%
