...
length(unique(openalex_pubmed_affil$primary_id))

affil_2024 <- openalex_pubmed_affil %>%
  filter(primary_id %in% papers_2024$primary_id)
length(unique(affil_2024$primary_id))
...
top_country <- openalex_pubmed_affil %>%
  ...
  distinct(primary_id, institution_country_code, .keep_all = TRUE) %>%
  count(institution_country_code, sort = TRUE)
...
top_country %>% 
  ...
  cols_label(
    institution_country_code = "Country Code",
    n = "Number of Papers"
  ) %>%
...
affil_country <- openalex_pubmed_affil %>%
  filter(!is.na(institution_country_code)) %>%
  select(primary_id, institution_country_code) %>%
...
country_year_counts <- country_year_data %>%
  distinct(primary_id, institution_country_code, publication_year) %>%
  count(institution_country_code, publication_year)
...
top_country_years <- country_year_counts %>%
  filter(institution_country_code %in% top_10_countries) %>%
